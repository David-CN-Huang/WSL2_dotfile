...
function Log:set_level(level)
  ...
  local status_ok, logger = pcall(require("structlog").get_logger, "MyCode")
  if status_ok then
  ...
  require("packer.log").new { level = mycode.log.level }
end
...
function Log:init()
  ...
  local log_level = Log.levels[(mycode.log.level):upper() or "WARN"]
  local mycode_log = {
    MyCode = {
      sinks = {
  ...
  structlog.configure(mycode_log)
  local logger = structlog.get_logger "MyCode"

  -- Overwrite `vim.notify` to use the logger
  if mycode.log.override_notify then
...
function Log:get_path()
  return string.format("%s/%s.log", get_dir_cache(), "mycode")
end
