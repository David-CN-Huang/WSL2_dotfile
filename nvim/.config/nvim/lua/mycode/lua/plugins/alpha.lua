...
function M.config()
  local mycode_dashboard = require "core.alpha.dashboard"
  local mycode_startify = require "core.alpha.startify"
  mycode.infras.alpha = {
    dashboard = { config = {}, section = mycode_dashboard.get_sections() },
    startify = { config = {}, section = mycode_startify.get_sections() },
    active = true,
    mode = "dashboard",
  }
end
...
local function resolve_config(theme_name)
  ...
  local section = mycode.infras.alpha[theme_name].section
...
local function configure_additional_autocmds()
  ...
  if not mycode.infras.lualine.options.globalstatus then
    aucmds[#aucmds + 1] =
...
function M.setup()
  ...
  local mode = mycode.infras.alpha.mode
  local config = mycode.infras.alpha[mode].config
